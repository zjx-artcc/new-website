...
<body>
	<div class="flex flex-wrap justify-center">
		<a href="/stats">
			<Card title="Top Controller of {currentMonthName}" subtext="{data.stats[0].first_name} {data.stats[0].last_name}" timestamp="{data.stats[0].month_three} HOURS" icon="ant-design:hourglass-twotone" />
		</a>
		<a href="/events">
			<Card title="Next Event" subtext={data.events[0].name} timestamp={new Date(data.events[0].event_start).toLocaleString(undefined, { month: 'short',day: 'numeric',year: 'numeric',hour: 'numeric',minute: 'numeric',timeZoneName: 'short'})} icon="ion:calendar"/>
		</a>
		<a href="/roster">
							...
							<tbody>
								<tr class="pt-4">
									<td style="width:30%;" class="pt-4">
										<Icon icon="ic:outline-warning" color="orange" class="text-center align-middle items-center"/>
									</td>
									<td style="width:70%;" class="pt-4">NOTAM:
										<p>GTOUT Of 2023 signups needed! Please sign up if you can control!</p>
									</td>
								</tr>
								<NewsCard date="December 29, 2023" description="BLAHB LAH BLAH IG UESS THIS WORKS"/>
							</tbody>
						</table>
						<span class=" mt-4 block text-center justify-center items-center">For More Information,
							<a href="discord.gg" style="width: 100%; text-decoration: none; color: blue;">
								Join the Discord!
							</a>
						</span>
					</div>
									...
									<tr style="background-color: rgba(255, 255, 255);">
										<td style="width: 50%;" align="left">
											<a href="/events/{data.events[0].id}/">
												<img loading="lazy" width="100%" src={data.events[0].banner} alt={data.events[0].name}/>
											</a>
										</td>
										<td style="width: 50%;">
											<a href="/events/{data.events[0].id}/">
											...
											<div style="text-align:center; font-size: 20px;">
												Hosted by: {data.events[0].host}
											</div>
											<br />
											<div style="text-align:center; font-size: 20px;">
												{new Date(data.events[0].event_start).toLocaleString(undefined, {month: 'short',day: 'numeric',year: 'numeric',hour: 'numeric',minute: 'numeric',timeZoneName: 'short'})}
											</div>
										</td>
									</tr>
									<tr>
									...
									<tr style="background-color: rgba(255, 255, 255);">
										<td style="width: 50%;" align="left">
											<a href="/events/{data.events[1].id}/">
												<img width="100%" src={data.events[1].banner} alt={data.events[1].name}/>
											</a>
										</td>
										<td style="width: 50%;">
											<a href="/events/{data.events[1].id}/">
											...
											<div style="text-align:center; font-size: 20px;">
												Hosted by: {data.events[1].host}
											</div>
											<br />
											<div style="text-align:center; font-size: 20px;">
												{new Date(data.events[1].event_start).toLocaleString(undefined, {month: 'short',day: 'numeric',year: 'numeric',hour: 'numeric',minute: 'numeric',timeZoneName: 'short'})}
											</div>
										</td>
									</tr>
									<tr>
									...
									<tr style="background-color: rgba(255, 255, 255);">
										<td style="width: 50%;" align="left">
											<a href="/events/{data.events[2].id}/">
												<img width="100%" src={data.events[2].banner} alt={data.events[2].name}/>
											</a>
										</td>
										<td style="width: 50%;">
											<a href="/events/{data.events[2].id}/">
											...
											<div style="text-align:center; font-size: 20px;">
												Hosted by: {data.events[2].host}
											</div>
											<br />
											<div style="text-align:center; font-size: 20px;">
												{new Date(data.events[2].event_start).toLocaleString(undefined, {month: 'short',day: 'numeric',year: 'numeric',hour: 'numeric',minute: 'numeric',timeZoneName: 'short'})}
											</div>
										</td>
									</tr>
									<tr>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						...
						<div class="flex flex-col items-center">
							<div id="1st-controller" class="inline-flex">
								...
								<div class="font-bold text-gray-800 mr-2 text-right">
									{data.stats[0].first_name}
									{data.stats[0].last_name}
								</div>
							</div>
							<div id="2nd-controller" class="inline-flex">
								<span class="text-gray-500">★★</span>
								<div class="font-bold text-gray-800 mr-2">
									{data.stats[1].first_name}
									{data.stats[1].last_name}
								</div>
							</div>
							<div id="3rd-controller" class="inline-flex">
								<span class="text-red-500">★</span>
								<div class="font-bold text-gray-800 mr-2">
									{data.stats[2].first_name}
									{data.stats[2].last_name}
								</div>
							</div>
						</div>
					</div>
				...
				<div class="bg-white shadow p-4">
					...
					<table style="border-spacing: 0; border-collapse: collapse; width: 100%;" class="mt-9">
						<tbody>
							<tr>
								<td style="font-size: 14px; width: 50%;" align="left">
									{data.newController[0].first_name}
									{data.newController[0].last_name} ({data.newController[0].rating})
								</td>
								<td style="font-size: 14px; width: 50%;" align="right">
									Joined on {new Date(data.newController[0].created_at).toLocaleDateString(undefined,{ month: 'short', day: 'numeric', year: 'numeric' })}
								</td>
							</tr>
							<tr>
								<td style="font-size: 14px; width: 50%;" align="left">
									{data.newController[1].first_name}
									{data.newController[1].last_name} ({data.newController[1].rating})
								</td>
								<td style="font-size: 14px; width: 50%;" align="right">
									Joined on {new Date(data.newController[1].created_at).toLocaleDateString(undefined,{ month: 'short', day: 'numeric', year: 'numeric' })}
								</td>
							</tr>
							<tr>
								<td style="font-size: 14px; width: 50%;" align="left">
									{data.newController[2].first_name}
									{data.newController[2].last_name} ({data.newController[2].rating})
								</td>
								<td style="font-size: 14px; width: 50%;" align="right">
									Joined on {new Date(data.newController[2].created_at).toLocaleDateString(undefined,{ month: 'short', day: 'numeric', year: 'numeric' })}
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="py-[30px] js-appear-enabled animated fadeIn" data-toggle="appear">
				<div class="justify-content-center py-[10px]">
					<div class="iconStar">
						...
						<a class="btn btn-hero btn-noborder btn-rounded btn-primary mb-10 items-center justify-center gap-2 js-appear-enabled animated fadeInUp" data-toggle="appear" data-class="animated fadeInUp" href="/feedback/">
							<Icon icon="tabler:pencil" class="inline-block text-lg" /> 
							Provide Feedback
						</a>
					</div>
				</div>
		...
		<div class="content content-full">
			...
			<div class="font-size-xs clearfix border-t pt-20 pb-10">
				...
				<div class="float-right">
					<a href="https://github.com/ZJX-ARTCC/jacksonville-issue-tracker/issues/new/choose" target="_blank"><i class="fas fa-bug text-pulse" /> Log a bug or Request a feature</a>
				</div>
			</div>
	</footer>
</body>