...
export const POST = async ({ request, cookies }): Promise<Response> => {
	...
	if (user.rating < 4) {
		return new Response('Your rating is too low (S2 or lower) to visit.', { status: 401 });
	} else if (user.rostered) {
		return new Response('You are already apart of ZJX!', { status: 401 });
	} else {
		try {
			...
			if (checkRequest.length > 0) {
				return new Response('You already have an open visit request.', {
					status: 401
				});
			}
